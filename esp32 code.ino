...
unsigned long myChannelNumber = 3281436; // Example: 1234567 (No quotes around numbers)
const char * myWriteAPIKey = "RIFGX7XLLQ11K4SN";   // Put your Write API Key inside the quotes

// --- OBJECT INITIALIZATIONS ---
DHT dht(DHTPIN, DHTTYPE);
LiquidCrystal_I2C lcd(0x27, 16, 2);
WiFiClient client; // Required for ThingSpeak
...
void loop() {
  ...
  if ((millis() - lastTime) > timerDelay) {
    ...
    // Set the fields with our sensor data
    ThingSpeak.setField(1, temp);
    ThingSpeak.setField(2, hum);
    ThingSpeak.setField(3, air);
    
    // Send the text status as a ThingSpeak status update
  ...
  delay(2000); // Wait 2 seconds before checking sensors and LCD again
}
